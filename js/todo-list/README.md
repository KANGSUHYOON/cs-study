...
- TODO 예정 날짜와 완료 시각 저장
- 날짜별 TODO 조회와 월별 캘린더 집계
...
| `routes/calendarRoutes.js` | 월별 캘린더 API 경로 연결 |
| `controllers/calendarController.js` | 월별 날짜 집계 조회와 응답 정규화 |
| `utils/dateUtils.js` | 날짜 유효성 검사와 월 시작·종료일 계산 |
| `sql/20260717_add_calendar_fields.sql` | 기존 Todo 데이터를 보존하는 캘린더 필드 마이그레이션 |
| `tests/todos.http` | REST API 직접 테스트 |
| `tests/calendar.http` | 날짜 및 캘린더 API 직접 테스트 |
...
기존 데이터를 유지하면서 `scheduled_date`, `completed_at`과 날짜 인덱스를 추가하려면
서버 실행 전에 다음 SQL 파일을 검토하고 MySQL에서 한 번만 실행합니다.

```text
study-todo-api/sql/20260717_add_calendar_fields.sql
```

이 파일은 자동 실행되지 않으며 반복 실행하면 중복 컬럼 또는 인덱스 오류가 발생할 수 있습니다.
...
| 특정 날짜 TODO 조회 | `GET` | `/todos?date=YYYY-MM-DD` | `SELECT` |
| TODO 추가 | `POST` | `/todos` | `INSERT` |
| 완료 상태 수정 | `PATCH` | `/todos/:id` | `UPDATE` |
| 제목 수정 | `PATCH` | `/todos/:id/title` | `UPDATE` |
| 예정 날짜 수정 | `PATCH` | `/todos/:id/date` | `UPDATE` |
| TODO 삭제 | `DELETE` | `/todos/:id` | `DELETE` |
| 월별 캘린더 집계 | `GET` | `/calendar?year=YYYY&month=M` | `SELECT` |
